			...
			[
				...
				},
				{
					...
					handler: (e: Event) => {
						...
						if (!this.isFinished || tag === 'IMG' || tag === 'BUTTON') { return; }

						if (this.isChecked) { inner.getDOM()?.classList.add('interesInner-grey'); } else { inner.getDOM()?.classList.remove('interesInner-grey'); }
